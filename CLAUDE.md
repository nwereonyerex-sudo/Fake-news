...
- **`train(epochs=0)` silently saved an untrained, randomly-initialized model** with no error (`best_val_loss` stayed `inf`, but nothing checked that). **`train(patience=0)` stopped after epoch 1 unconditionally**, even when epoch 1 was itself an improvement — the "epochs without improvement" counter gets reset to 0 right after a best-epoch, so `0 >= patience(=0)` was immediately true. Both confirmed by direct reproduction, not just inspection. *Fix:* `train()` now raises `ValueError` for `epochs < 1` or `patience < 1` before doing any work.

### Testing / operational safety

- **A quick sanity-check call to `train()` during testing silently overwrote the real, full-dataset, early-stopped production model** at `models/fake_news_classifier.joblib` with a throwaway 2,000-row test run's — `save_artifact()` always wrote to the same default path with no way for `train()` callers to redirect it. Caught by checking the saved artifact's `metadata` (`train_size: 1600` instead of the expected ~35,900) after the "just testing" run. *Fix:* `train()` now takes an optional `save_path` parameter; pass a scratch path whenever calling `train()` for anything other than a real training run you intend to keep. **Rule of thumb: never call `train()` with its default save path unless you mean to replace the production model.**
